 ...
 *
 * user_shader parameter provides external shader usage. replaces font shader,
 * generator shader is still static.
 *
 * Returns a new MSDF GL context, or NULL if creating the context failed.
 */
MSDFGL_EXPORT msdfgl_context_t msdfgl_create_context(const char *version, GLuint user_shader);
